...
void applyNodeTransformation(std::vector<Vertex>& vertices, const tinygltf::Node& node) {
    ...
    for(const tinygltf::Mesh& gltfMesh : model.meshes) {
        ...
        for(const tinygltf::Primitive& primitive : gltfMesh.primitives) {
            ...
            if(primitive.attributes.contains("POSITION")) {
                ...
                const auto* positions = reinterpret_cast<const glm::vec3*>(&buffer.data[bufferView.byteOffset + accessor.byteOffset]);
            ...
            if(primitive.attributes.contains("NORMAL")) {
                ...
                const tinygltf::BufferView& bufferView = model.bufferViews[accessor.bufferView];
                const tinygltf::Buffer& buffer = model.buffers[bufferView.buffer];
                const auto* normals = reinterpret_cast<const glm::vec3*>(&buffer.data[bufferView.byteOffset + accessor.byteOffset]);
            ...
            if(primitive.attributes.contains("TEXCOORD_0")) {
                ...
                const tinygltf::BufferView& bufferView = model.bufferViews[accessor.bufferView];
                const tinygltf::Buffer& buffer = model.buffers[bufferView.buffer];
                const auto* texCoords = reinterpret_cast<const glm::vec2*>(&buffer.data[bufferView.byteOffset + accessor.byteOffset]);
            ...
            if(primitive.attributes.contains("WEIGHTS_0")) {
                ...
                const tinygltf::BufferView& bufferView = model.bufferViews[accessor.bufferView];
                const tinygltf::Buffer& buffer = model.buffers[bufferView.buffer];
                const auto* weights = reinterpret_cast<const glm::vec4*>(&buffer.data[bufferView.byteOffset + accessor.byteOffset]);
        ...
        /*
         * GLTF only stores raw vertex positions (at center, without any transformation). Transformations are stored in
         * nodes, so we get the node corresponding to this mesh, and apply transformation to the vertices so that they are
